...
function isInInterval(date) {
    const fields = this._fields;
    if (
        !(
            (matchSchedule(date.getMonth() + 1, fields.month) &&
                matchSchedule(date.getDate(), fields.dayOfMonth)) ||
            (fields.dayOfWeek.length !== 8 &&
                matchSchedule(date.getDay(), fields.dayOfWeek))
        )
    ) {
        return false;
    }
    if (!matchSchedule(date.getHours(), fields.hour)) {
        return false;
    }
    if (!matchSchedule(date.getMinutes(), fields.minute)) {
        return false;
    }
    if (
        !(
            fields.second.length === 1 ||
            matchSchedule(date.getSeconds(), fields.second)
        )
    ) {
        return false;
    }
...
function CronExpression(fields, options) {
    ...
    this._startDate = options.startDate
        ? new CronDate(options.startDate, this._tz)
        : null;
    this._endDate = options.endDate
        ? new CronDate(options.endDate, this._tz)
        : null;
    this._fields = fields;
...
CronExpression._parseField = function _parseField(field, value, constraints) {
    ...
    function parseSequence(val) {
        ...
        var atoms = val.split(",").sort(function (a, b) {
    ...
    function parseRange(val, repeatInterval) {
        ...
        if (atoms.length > 1) {
            ...
            if (safeIsNaN(repeatIndex) || repeatIndex <= 0) {
                throw new Error(
                    "Constraint error, cannot repeat at every " +
                        repeatIndex +
                        " time."
                );
            }
...
CronExpression.prototype._applyTimezoneShift = function (
    currentDate,
    dateMathVerb,
    method
) {
...
CronExpression.prototype._findSchedule = function _findSchedule(reverse) {
    ...
    if (this.isInInterval(currentDate, this._fields)) {
        while (stepCount < LOOP_LIMIT) {
            ...
            // Match second
            if (this._fields.second.length !== 1) {
                if (
                    matchSchedule(currentDate.getSeconds(), this._fields.second)
                ) {
                    this._applyTimezoneShift(
                        currentDate,
                        dateMathVerb,
                        "Second"
                    );
            ...
            // Match minute
            if (this._fields.minute.length !== 60) {
                if (
                    matchSchedule(currentDate.getMinutes(), this._fields.minute)
                ) {
                    this._applyTimezoneShift(
                        currentDate,
                        dateMathVerb,
            ...
            if (this._fields.hour.length !== 24) {
                if (matchSchedule(currentHour, this._fields.hour)) {
                    if (this._dstStart !== currentHour) {
                        ...
                        matchSchedule(currentHour - 1, this._fields.hour)
                    ) {
            ...
            var dayOfMonthMatch = matchSchedule(
                currentDate.getDate(),
                this._fields.dayOfMonth
            );
            var dayOfWeekMatch = matchSchedule(
                currentDate.getDay(),
                this._fields.dayOfWeek
            );

            var isDayOfMonthWildcardMatch = isWildcardRange(
                this._fields.dayOfMonth,
                CronExpression.constraints[3]
            );
            var isDayOfWeekWildcardMatch = isWildcardRange(
                this._fields.dayOfWeek,
                CronExpression.constraints[5]
            );

            // Add or subtract day if select day not match with month (according to calendar)
            if (
                this._fields.dayOfMonth.length !== 31 ||
                this._fields.dayOfWeek.length !== 8
            ) {
                if (
                    dayOfMonthMatch ||
                    (this._fields.dayOfWeek.length !== 8 && dayOfWeekMatch)
                ) {
            ...
            // Match month
            if (this._fields.month.length !== 12) {
                if (
                    matchSchedule(
        ...
        while (stepCount < LOOP_LIMIT) {
            ...
            if (
                !isDayOfMonthWildcardMatch &&
                isDayOfWeekWildcardMatch &&
                !dayOfMonthMatch
            ) {
                this._applyTimezoneShift(currentDate, dateMathVerb, "Day");
                continue;
            ...
            if (
                !(isDayOfMonthWildcardMatch && isDayOfWeekWildcardMatch) &&
                !dayOfMonthMatch &&
                !dayOfWeekMatch
            ) {
                this._applyTimezoneShift(currentDate, dateMathVerb, "Day");
            ...
            // Match month
            if (
                !matchSchedule(currentDate.getMonth() + 1, this._fields.month)
            ) {
...
CronExpression.parse = function parse(expression, options, callback) {
    ...
    function parse(expression, options) {
        ...
        if (mappedFields.month.length === 1) {
            var daysInMonth =
                CronExpression.daysInMonth[mappedFields.month[0] - 1];

            if (mappedFields.dayOfMonth[0] > daysInMonth) {
                throw new Error("Invalid explicit day of month definition");
            }

            mappedFields.dayOfMonth = mappedFields.dayOfMonth.filter(function (
                dayOfMonth
            ) {
        ...
        function parseNthDay(val) {
            ...
            if (atoms.length > 1) {
                ...
                if (/,/.test(val)) {
                    throw new Error(
                        "Constraint error, invalid dayOfWeek `#` and `,` " +
...
CronExpression.prototype.isInInterval = isInInterval;
