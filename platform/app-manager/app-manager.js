 ...
 * @returns {string}
 */
function generateTopic(appPath, appName) {
    ...
    // use the timestamp and application's name to create topic
    let dirName = path.dirname(appPath).split(path.sep).pop();
    hash.update(dirName + appName);
    ...
    if(appData.appPath && appData.metadataPath) {
        codeContainer.setupAppRuntimeEnvironment(appData.appPath, appData.metadataPath)
            .then((newAppPath) => {
                ...
                let appTopic  = generateTopic(newAppPath, appName);
